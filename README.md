...
### Экземпляр класса
Вы можете указать любую функцию-конструктор в качестве типа. По-умолчанию валидатор проверит соответствие через `instanceof`, так что, для экземпляров наследников, валидация тоже будет проходить:
```javascript
class User {
